...
def train_plot(features, labels):
    ...
    model = learn_model(1, features[:,(0,2)], labels)
    C = apply_model(np.array(np.vstack([X.ravel(), Y.ravel()]).T), model).reshape(X.shape)
    if COLOUR_FIGURE:
    ...
    if COLOUR_FIGURE:
        ...
        for lab,ma in zip(range(3),"Do^"):
            plt.plot(features[labels == lab, 0],features[lab==lab,2],ma,c=(1.,1.,1.))
